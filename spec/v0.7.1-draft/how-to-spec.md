...
5. **Finalize release**: Open a pull request to update all of the "current" version pointers in the `spec` repo:
   ...
   - Update `/public/_redirects` line 2 to point spec.canvasprotocol.org to:
     ...
     ```
   - Excluding the `/spec` directory, find and replace the previous version with the new version (e.g., replace `v0.5` with `v0.7.1`)
     - This will update the Cookbook, Catalog, Examples, and README.md

6. **Create git tag**: After merging, create and push a git tag for the release (e.g., `git tag v0.7.1 && git push origin v0.7.1`)
